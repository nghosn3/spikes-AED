function [all_dose_curves,all_tHr,ptIDs,all_med_names,all_ieeg_offset,max_dur,emu_dur] = get_aed_curve_min_dose(ptIDs)
    ...
    for n = 1:length(med_names)
        ...
        if sum(med_ind)==0
            ...
            elseif sum(ind)~=0 && ~isnan(pt_home_meds.dose(ind))
                ...
                [~,css]=get_c0_min_dose(dose,frequency,tHalf,F,vd,tmax,ka,min_dose); %this is curve of one day, dose as a fraction of minimum dose
                if this_med_schedule(1)==0 % if meds admisitered together
                    tInt=this_med_schedule(2);
                else
                    tInt = this_med_schedule(1);
                end
                c0=css(round((mod(tInt,24./frequency))*60)); %this is the c0 at the first admin,indexed from t=0 (midnight) so need to adjust based on sleep/wake estimate
            else
                c0=0;
            end
            
            %% get rest of drug curve
            c_drug=[];
            ...
            while i <= height(this_med_info)
                ...
                if tInt < 0.08 % 5 minutes apart
                    dose = dose + this_med_info.dose(i);

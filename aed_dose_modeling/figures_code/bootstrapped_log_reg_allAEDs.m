...
for i=1:length(ptIDs) 
    ...
    for n =1:length(med_names)
        ...
        if ~isempty(drug)
            ...
            % until the drug starts, replace with steady state level unless ativan - for ativan, make it 0
            if contains(med_names(n),'lorazepam')
                drugs(n,1:dStart) = zero(1,dStart);
            elseif dStart >24 % if the drug was administered later in the EMU stay; otherwise keep as nan
                drugs(n,24:dStart) = zeros(1,dStart-24+1);
            end
        end
    end
    ...
    features(empty_inds,i)={zeros(1,nbins)}; %adding small number to make columns full rank
...
if remove_nan_rows
    array_drug_table = table2array(drug_table(:,top_meds));
...
% for ASM model
model = 1;
feat_names_new = [top_meds; {'t_last_seizure'}; {'ptID'}; {'seizure'}]; 
feat_names_new = [top_meds; {'ptID'}; {'seizure'}]; 

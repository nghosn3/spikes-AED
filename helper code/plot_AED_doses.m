...
for y = 1:length(ptIDs)
    ...
    %plot doses of medication over EMU stay- some include alcohol administered
    figure();
    % convert time axis to hours
    plot(days*24,each_drug_dose,'.-'); hold on;
    plot(days*24,total_dose,'k','LineWidth',1.5);
    ...
    title([ptID ': AED mg/day during EMU stay']); ylabel('dose (mg)'); xlabel('time (hrs) of EMU stay');
    ...
    if plot_bp_soz
        ...
        title('avg theta power'); legend([{'non-soz electrdoes'},{'soz electrodes'}])
        ylim(ylimits)
        
        subplot(3,2,3)
        plot(tHr,movAvg(nonsoz_alphas));
        hold on;plot(tHr,movAvg(soz_alphas));
        title('avg alpha power'); legend([{'non-soz electrdoes'},{'soz electrodes'}])
        ylim(ylimits)
        
        subplot(3,2,4)
        plot(tHr,movAvg(nonsoz_betas));
        hold on;plot(tHr,movAvg(soz_betas));
        title('avg beta power'); legend([{'non-soz electrdoes'},{'soz electrodes'}])
        ylim(ylimits)
        
        subplot(3,2,5)
        plot(tHr,movAvg(nonsoz_gamma1));
        hold on;plot(tHr,movAvg(soz_gamma1));
        title('avg gamma (30-50Hz) power'); legend([{'non-soz electrdoes'},{'soz electrodes'}])
        ylim(ylimits)
        
        subplot(3,2,6)
        plot(tHr,movAvg(nonsoz_gamma2));
        hold on;plot(tHr,movAvg(soz_gamma2));
        title('avg gamma (50-80Hz?) power'); legend([{'non-soz electrdoes'},{'soz electrodes'}])
        ylim(ylimits)

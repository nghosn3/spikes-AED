...
axis square;
set(gca,'fontsize',14);
title('MSE (actual-forcasted)');
legend('model with ASM load','model w/out ASM load');
ylabel('# patients');
xlabel('MSE')

nexttile;
%plot(residuals_asm,residuals_null,'.',MarkerSize=20)
histogram(residuals_asm,'FaceAlpha', 0.5,'Facecolor','black','EdgeColor', 'black', 'linewidth', 2); hold on;
histogram(residuals_null,'FaceAlpha', 0.5,'Facecolor','red','EdgeColor', 'red', 'linewidth', 2); hold on;
legend('model with ASM load','model w/out ASM load');
title('Residuals (actual-forcasted)');
ylabel('# patients');
axis square;
...
% nexttile;
% %plot(residuals_asm,residuals_null,'.',MarkerSize=20)
% boxchart([residuals_asm',residuals_null']); hold on;
% %legend('model with ASM load','model w/out ASM load');
% title('Residuals (actual-forcasted)');
% xticklabels({'model with ASM load','model w/out ASM load'})
% ylabel('residuals')
% axis square;
% set(gca,'fontsize',14);
% %ylabel('MSE null model')

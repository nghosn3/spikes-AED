...
%load spike rate - new from 10/22/21 (samp/10min)
load('all_spike_rate_new.mat');
inds = cellfun('isempty',all_spike_rate);
all_spike_rate(inds)=[];
inds = cellfun('isempty',ptIDs);
ptIDs(inds)=[];
...
for ipt = 1:length(ptIDs)
    ...
    ptID =  ptIDs{ipt};
        ...
        addpath([pwd '/spikes-AED/results/change_point/downsamp_spikes_new'])
